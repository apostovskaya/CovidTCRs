...
assert len(
    sys.argv) == 5, "The input is incorrect. Expected: (1) script name; (2) main folder; " \
...
file_in = sys.argv[
    2]  # 'TCRex_processed/per_patient/new_intersected_double_TCRcounts_per_virus.tsv'
...
print(
    "~ The output file will be saved in the same directory as the input file: " + file_in_dir)
...
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'INZO', 'disease_severity'] = 'critical'
tcr_per_virus_inter.sort_values(
    by=['disease_severity', 'day', 'new_id', 'T-cell_type'], inplace=True)
...
sorterIndex = dict(zip(sorter, range(
    len(sorter))))  # {'asymptomatic': 0, 'moderate': 1, 'critical': 2}
# Generate a rank column that will be used to sort the dataframe numerically
tcr_per_virus_inter['group_Rank'] = tcr_per_virus_inter[
    'disease_severity'].map(sorterIndex)
tcr_per_virus_inter.sort_values(
    by=['group_Rank', 'day', 'new_id', 'T-cell_type'], inplace=True)
...
tcr_per_virus_inter['Hits_SARS-CoV-2'] = tcr_per_virus_inter[
                                             'N_SARS-CoV-2_only_TCRs'] / \
                                         tcr_per_virus_inter[
                                             'unique_TCRs']
tcr_per_virus_inter['Hits_CoV'] = tcr_per_virus_inter['N_CoV_TCRs'] / \
                                  tcr_per_virus_inter['unique_TCRs']
tcr_per_virus_inter['Hits'] = (tcr_per_virus_inter['N_SARS-CoV-2_only_TCRs'] +
                               tcr_per_virus_inter['N_CoV_TCRs']) / \
                              tcr_per_virus_inter['unique_TCRs']
print(
...
tcr_per_virus_inter = pd.merge(tcr_per_virus_inter, age,
                               on=['study_patient_id', 'disease_severity'],
                               how='left')
# no age data for patients 'imseq27', 'imseq25', 'imseq28'
no_age_info = tcr_per_virus_inter.loc[
    tcr_per_virus_inter['age'].isna(), 'study_patient_id'].unique()
print(
...
no_comorb_info = tcr_per_virus_inter.loc[
    tcr_per_virus_inter['comorbidities'].isna(), 'study_patient_id'].unique()
print(
    f'No information about comorbidities is available for the following patients: \n{no_comorb_info}')
...
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'study_patient_id'] == 'imseq28', 'disease_severity'] = 'severe'

# add disease severity category label to compare with IR-Binder data
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'asymptomatic', 'disease_burden'] = 'low'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'moderate', 'disease_burden'] = 'medium'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'severe', 'disease_burden'] = 'medium'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'critical', 'disease_burden'] = 'high'

# add critical disease label to compare with IR-Binder data
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'asymptomatic', 'critical_disease'] = 'No'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'moderate', 'critical_disease'] = 'No'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'severe', 'critical_disease'] = 'No'
tcr_per_virus_inter.loc[tcr_per_virus_inter[
                            'disease_severity'] == 'critical', 'critical_disease'] = 'Yes'

tcr_per_virus_inter = tcr_per_virus_inter[
    ['new_id', 'patient_id', 'T-cell_type', 'study_patient_id',
...
df = tcr_per_virus_inter.drop_duplicates(subset='study_patient_id',
                                         ignore_index=True)
...
ages_median['medium'] = df.loc[
    df['disease_burden'] == 'medium', 'age'].median()
ages_median['low'] = df.loc[df['disease_burden'] == 'low', 'age'].median()
ages_median['critical'] = df.loc[
    df['critical_disease'] == 'Yes', 'age'].median()
ages_median['not_critical'] = df.loc[
    df['critical_disease'] == 'No', 'age'].median()
...
tcr_per_virus_inter = tcr_per_virus_inter[
    tcr_per_virus_inter['disease_severity'] != 'asymptomatic']
tcr_per_virus_inter.to_csv(
    path_or_buf=f'{folder}/{file_in_dir}/{prefix_out}_{filename_in}.tsv',
    sep='\t', index=False)
...
# Metadata for Zenodo
# meta = pd.read_csv(
#     '/Users/apost/Documents/CloudMail/PhD_2020/IMSEQ/TCRex_processed/per_patient/MetaForPlotting_new_intersected_double_TCRcounts_per_virus.tsv',
#     sep='\t')
# meta = meta[['patient_id', 'T-cell_type', 'study_patient_id',
#              'disease_severity', 'critical_disease', 'day', 'week',
#              'total_count', 'unique_TCRs', 'unique_CDR3s', 'age',
#              'comorbidities']]
# meta.rename(columns={'patient_id': 'sample_id',
#                      'study_patient_id': 'patient_id'}, inplace=True)
# meta.sort_values(by='sample_id', inplace=True)
# meta.to_csv('/Users/apost/Documents/CloudMail/PhD_2020/IMSEQ/IMSEQ_Metadata.tsv', sep='\t', index=False)