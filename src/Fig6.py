...
ax = sns.lineplot(data=all_data_scaled_week_max, x='week', y=parameter,
                  ...
                  hue_order=['Critical', 'Non-critical'],
                  hue='critical_disease',
                  palette={'Non-critical': 'tab:blue',
                           'Critical': 'tab:orange'})
# y min and max
ymin, ymax = ax.get_ylim()
plt.vlines(x=4, ymin=ymin, ymax=ymax,
           colors='tab:blue', ls='--', lw=0.75,
           label='Recovery of non-critical group')
plt.ylabel(parameter)
plt.legend(title='COVID-19 severity', frameon=False)
...
if p > alpha:
    print(
        f'The same distribution of {parameter} between active and recovered non-critical patients (fail to reject H0)')
else:
    print(
        f'Different distribution of {parameter} between active and recovered non-critical patients (reject H0)')
